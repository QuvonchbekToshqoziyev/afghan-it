...
type Course = TranslatedContent & { id: string; category: string; level: string; accessTier: string };
...
export default function CoursesPage() {
  ...
  useEffect(() => { const params = new URLSearchParams(window.location.search); setQuery(params.get('search') || ''); setCategory(params.get('category') || ''); fetch(`${api}/courses`).then(r => r.ok ? r.json() : []).then((items: Course[]) => setCourses(items.map(item => localize(item, language)))).catch(() => setCourses([])); }, [language]);
