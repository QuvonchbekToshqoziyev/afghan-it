...
export default function TeacherPage() {
  ...
  const [learners, setLearners] = useState<{ learner: { name: string; email: string }; enrollment: { status: string } }[]>([]);
  ...
  useEffect(() => { const accessToken = token(); if (!accessToken) return; const headers = { authorization: `Bearer ${accessToken}` }; fetch(`${api}/teacher/dashboard`, { headers }).then((response) => response.ok ? response.json() : null).then(setSummary); fetch(`${api}/teacher/courses`, { headers }).then((response) => response.ok ? response.json() : []).then(setCourses); }, []);
  async function showLearners(courseId: string) { const accessToken = token(); const response = await fetch(`${api}/teacher/courses/${courseId}/learners`, { headers: { authorization: `Bearer ${accessToken}` } }); setLearners(response.ok ? await response.json() : []); }
